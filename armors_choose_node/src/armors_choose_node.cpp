...
public:
    CompareArmors() : Node("CompareArmors")
    {
        ...
        armors_sub_first_ = this->create_subscription<auto_aim_interfaces::msg::Armors>(
            "/detector/armors_first", rclcpp::SensorDataQoS(),
            std::bind(&CompareArmors::armors_first_callback, this, std::placeholders::_1));
        armors_sub_second_ = this->create_subscription<auto_aim_interfaces::msg::Armors>(
...
private:
    ...
    {
        if (armors_first->armors.size() > 0) // if there is armor detected
        {
    ...
    {
        if (armors_second->armors.size() > 0) // if there is armor detected
        {
            if (time_1_ == 0)
            {
                time_2_ = 80;
    ...
    rclcpp::Subscription<auto_aim_interfaces::msg::Armors>::SharedPtr armors_sub_first_;
    rclcpp::Subscription<auto_aim_interfaces::msg::Armors>::SharedPtr armors_sub_second_;
